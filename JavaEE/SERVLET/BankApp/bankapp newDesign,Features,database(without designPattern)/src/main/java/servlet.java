    ...
    private void save(HttpServletRequest request, HttpServletResponse response) {
        if (request.getParameter("pho").isEmpty()) {
            ...
            for (int i = 0; i < transaction.size(); i++) {
                ...
                this.dao.save(transaction.get(i).getAmount(), transaction.get(i).getYear(), transaction.get(i).getPercent(), transaction.get(i).getBalance(), transaction.get(i).getPhone());
            }
            ...
            if(amount>0&&percent>0&&percent<=100&&finalamount>0&&finalamount>amount){
                ...
                if (transYear.contains(obj)) {
                    ...
                    request.setAttribute("transYear", transYear);
                        ...
                        this.dao.saveyears(transYear.get(transYear.size()-1).getAmount(), transYear.get(transYear.size()-1).getYear(), transYear.get(transYear.size()-1).getPercent(), transYear.get(transYear.size()-1).getBalance(), transYear.get(transYear.size()-1).getPhone(),transYear.get(transYear.size()-1).getYearpayback());
                    try {
                        request.getRequestDispatcher("/numofyears.jsp").forward(request, response);
                    } catch (Exception e) {
                        throw new RuntimeException(e);
            ...
            if(amount>0&&finalamount>0&&year>0&&year<50&&finalamount>amount){
                ...
                if (transPercent.contains(obj)) {
                    ...
                    request.setAttribute("transPercent", transPercent);
                        ...
                        this.dao.save(transPercent.get(transPercent.size()-1).getAmount(), transPercent.get(transPercent.size()-1).getYear(), transPercent.get(transPercent.size()-1).getPercent(), transPercent.get(transPercent.size()-1).getBalance(), transPercent.get(transPercent.size()-1).getPhone());
        ...
        if (request.getParameter("amount").isEmpty() || request.getParameter("year").isEmpty() || request.getParameter("percentage").isEmpty()) {
            ...
            if(amount>0&&percent>0&&percent<=100&&year>0&&year<=50){
                ...
                if (transNpv.contains(obj)) {
                    ...
                    transNpv.add(obj);
                    request.setAttribute("transNpv", transNpv);
                        transNpv.get(transNpv.size()-1).setPhone(loginphone);
                        this.dao.savenpv(transNpv.get(transNpv.size()-1).getAmount(), transNpv.get(transNpv.size()-1).getYear(), transNpv.get(transNpv.size()-1).getPercent(), transNpv.get(transNpv.size()-1).getPhone(), transNpv.get(transNpv.size()-1).getNpv(),transNpv.get(transNpv.size()-1).getNpvState());
                    try {
                        request.getRequestDispatcher("/npv.jsp").forward(request, response);
                    } catch (Exception e) {
                        throw new RuntimeException(e);
        ...
        if (request.getParameter("amount").isEmpty() || request.getParameter("year").isEmpty() || request.getParameter("percentage").isEmpty()) {
            ...
            if(amount>0&&percent>0&&percent<=100&&year>0&&year<=50){
                ...
                if (transactionlog.contains(obj)) {
                    ...
                    this.dao.save(transactionlog.get(transactionlog.size()-1).getAmount(), transactionlog.get(transactionlog.size()-1).getYear(), transactionlog.get(transactionlog.size()-1).getPercent(), transactionlog.get(transactionlog.size()-1).getBalance(), transactionlog.get(transactionlog.size()-1).getPhone());
                }
                try {
                    request.getRequestDispatcher("/finalcash.jsp").forward(request, response);
                } catch (Exception e) {
