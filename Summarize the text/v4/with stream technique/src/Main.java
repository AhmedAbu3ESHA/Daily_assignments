            ...
            public static void main(String[] args) {
                ...
                if (persent > 0 && persent <= 100) {
                    ...
                    List<Integer> counter = statmentsAL.stream().map(statement -> {
                                int w = calculateStatementWeight(statement, wordFrequencyMap);
                                System.out.println(statement.trim() + "" + w);
                                return w;
                            })
                            .collect(toList());
...
//                    Map<String, Integer> sentansewithweight = IntStream.range(0, counter.size())
//                            .boxed()
//                            .collect(toMap(i -> statmentsAL.get(i), i -> counter.get(i)));
...
//                            LinkedHashMap<String, Integer> sortedmap = scounter.stream().filter(j -> scounter.get(j) == sentansewithweight.get(key))
//                                    .collect(Collectors.toMap(keys -> key, j -> scounter.get(j), (e1, e2) -> e1, LinkedHashMap::new));
                        }
